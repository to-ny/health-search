 ...
 *
 * Batch processes all SmPC PDFs from the SAM export to extract excipient data.
 * Creates a JSON database that can be bundled with the application.
 *
 * Usage:
 ...
 *   --extract-only    Only extract URLs from SAM export, don't process PDFs
 */

import { createReadStream, existsSync, mkdirSync, unlinkSync, readFileSync, writeFileSync } from 'fs';
...
// Configuration
const CONFIG = {
  ampFile: 'data/sam-export/AMP-1768053603807.xml',
...
// ============================================================================
// Step 1: Extract AMP to SmPC URL mappings from SAM export
// ============================================================================

async function extractUrlMappings(): Promise<AmpSmpcMapping[]> {
  console.log('📦 Extracting SmPC URLs from SAM export...');

  const mappings: AmpSmpcMapping[] = [];
  const fileStream = createReadStream(CONFIG.ampFile);
...
async function main(): Promise<void> {
  ...
  if (existsSync(CONFIG.urlMappingFile) && !extractOnly) {
    ...
    mappings = await extractUrlMappings();
  }
